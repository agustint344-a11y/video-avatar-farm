/**
 * BUILD "arterias-elena" (Dra. Elena Vidal) — curcuma para las arterias. CLINIC. QR x6. Pool 48.
 ...
 * Salidas: src/VideoEdit/data/cues_arterias-elena.json  +  _v3/arterias-elena_avatarcuts.json
 */
import fs from "node:fs";
import path from "node:path";
const ROOT = process.cwd();
const SLUG = "arterias-elena";
const FPS = 30;
const T = "clinic";
...
const MAXWIN = sec(42);
const avatarBeats = [
  ["hay una especie amarilla", "que casi nadie te explica"],        // hook
  ["soy la doctora elena vidal", "que tenes que conocer"],          // presentación + retención
  ["antes de contarte el metodo", "el primer enlace de la descripcion"], // CTA1
  ["vino a verme una paciente", "la estaba absorbiendo"],           // caso Elsa
...
const compBeats = [
  ["el dano empieza con la inflamacion", 8, "MythVsTruth", { theme: T, myth: "Las arterias se tapan solo por el colesterol.", truth: "El daño empieza con la INFLAMACIÓN de la pared: eso la vuelve pegajosa y ahí entra el colesterol. Bajar la inflamación cuida la puerta." }],
  ["la inflamacion de tus arterias sube", 8.5, "Checklist", { theme: T, title: "Qué enciende la inflamación", items: ["Azúcar y harinas refinadas", "Ultraprocesados y frituras", "Cigarrillo y estrés crónico", "Sedentarismo"] }],
  ["y ese ingrediente es la pimienta negra", 8.5, "Steps", { theme: T, eyebrow: "El método que nadie explica", title: "Para que se absorba", steps: [{ title: "Pimienta negra: absorción ×2000", sub: "la piperina le abre la puerta" }, { title: "Con grasa buena (oliva, coco)", sub: "la curcumina es liposoluble" }, { title: "Con calor suave", sub: "la leche dorada la activa" }] }],
  ["puedo dejar la pastilla", 8, "MythVsTruth", { theme: T, myth: "Con cúrcuma puedo dejar la medicación del corazón.", truth: "JAMÁS. La cúrcuma acompaña, no reemplaza. Nunca dejes por tu cuenta lo que tu médico te indicó. Sumar, nunca restar." }],
  ["suma la curcuma a tus comidas", 8.5, "Steps", { theme: T, eyebrow: "Cómo hacerlo", title: "El plan en 3 pasos", steps: [{ title: "Sumala a tus comidas", sub: "guisos, arroz, huevos, verduras" }, { title: "Siempre pimienta + grasa buena", sub: "el trío que la activa" }, { title: "Probá la leche dorada, y sé constante", sub: "es un hábito, no urgencia" }] }],
  ["el secreto no es un suplemento", 8, "PullQuote", { theme: T, quote: "Cuidar tu corazón no siempre necesita cosas caras: empieza por usar bien lo que ya tenés en la cocina. El secreto es el método correcto y la constancia.", author: "Dra. Elena Vidal" }],
];
const components = [];
const compMiss = [];
...
const ovBeats = [
  ["soy la doctora elena vidal", 5, "LowerThird", { theme: T, accentText: "SALUD CON EVIDENCIA", title: "Dra. Elena Vidal", sub: "cúrcuma para tus arterias" }],
  ["un compuesto estrella que se llama curcumina", 3.8, "KeywordPop", { theme: T, word: "CURCUMINA", sub: "el antiinflamatorio estrella de la cúrcuma", pos: "center" }],
  ["hasta unas dos mil veces", 3.8, "KeywordPop", { theme: T, word: "PIMIENTA ×2000", sub: "la piperina multiplica la absorción", pos: "center" }],
  ["el cuidado de tus arterias", 5, "LowerThird", { theme: T, accentText: "BENEFICIO 1", title: "Arterias y corazón", sub: "menos inflamación, mejor circulación" }],
  ["es sobre el colesterol", 5, "LowerThird", { theme: T, accentText: "BENEFICIO 2", title: "Colesterol", sub: "ayuda al perfil y lo protege de oxidarse" }],
  ["la inflamacion de todo el cuerpo", 5, "LowerThird", { theme: T, accentText: "BENEFICIO 3", title: "Articulaciones", sub: "antiinflamatorio: menos rigidez" }],
  ["el azucar en sangre", 5, "LowerThird", { theme: T, accentText: "BENEFICIO 4", title: "Azúcar y antioxidante", sub: "sensibilidad a la insulina" }],
  ["si tomas anticoagulantes", 5.5, "Callout", { theme: T, icon: "⚠️", title: "Anticoagulantes", sub: "en dosis altas aumenta el riesgo de sangrado: consultá", tone: "warn" }],
  ["si tenes piedras en la vesicula", 5.5, "Callout", { theme: T, icon: "⚠️", title: "Vesícula y cirugía", sub: "puede empeorar cálculos; suspendela 2 semanas antes de operarte", tone: "warn" }],
  ["proba la leche dorada", 4.5, "SectionTitle", { theme: T, eyebrow: "LA RECETA", title: "Leche dorada" }],
  ["todo el detalle con la receta exacta de la leche", 6, "SplitInfo", { eyebrow: "En resumen", title: "Mi guía natural", items: ["La receta de la leche dorada", "Las cantidades diarias seguras", "Con qué comidas sumarla"] }],
  // ---- QR ×6: los 3 CTAs reales + 3 extras ----
  ["en los comentarios de este video", 7.5, "QRTag", { theme: T, corner: "bl", src: QR }],   // CTA1 (~5.6')
  ["el cuidado de tus arterias", 7, "QRTag", { theme: T, corner: "bl", src: QR }],            // extra (beneficio)
  ["fijada en los comentarios", 7.5, "QRTag", { theme: T, corner: "bl", src: QR }],           // CTA2 (~7.7')
  ["suma la curcuma a tus comidas", 7, "QRTag", { theme: T, corner: "bl", src: QR }],         // extra (pasos)
